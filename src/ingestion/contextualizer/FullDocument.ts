...
export class FullDocumentContextualizer implements Contextualizer {
  async contextualize(
    ...
    try {
      ...
      console.error(
        `Failed on cache-warming chunk ${firstChunk!.id}:`,
        error?.message || error
      );
    }
    ...
    const enrichedChunks = await Promise.all(
      remainingChunks.map(chunk =>
        limit(async () => {
          ...
          while (attempts < maxAttempts) {
            try {
              ...
              if (
                ...
                attempts++;
                console.warn(
                  `Rate limited on chunk ${chunk.id}. Sleeping for 30s... (Attempt ${attempts}/${maxAttempts})`
                );
                await sleep(30000);
              } else {
          ...
          console.error(
            `Giving up on chunk ${chunk.id} after ${maxAttempts} rate limit retries.`
          );
