...
const doc: Document = {
  id: "doc-synthetic-1",
  source: "test",
  content: `The Postgres query planner chooses execution strategies based on table statistics. It estimates row counts using histograms and most-common-value lists. When statistics are stale, the planner can pick a bad join order. Running ANALYZE regularly keeps these estimates fresh.

Bananas are a good source of potassium. They also contain fiber and vitamin B6. Many people eat them before exercise for quick energy. Unlike apples, they don't need refrigeration to stay fresh for a few days.

HNSW indexes trade build time for faster approximate nearest neighbor queries. IVFFlat is cheaper to build but has lower recall at the same speed. The choice between them depends on whether you can tolerate slower ingestion for better query-time performance. Most production RAG systems favor HNSW once the corpus is large enough to justify it.
...
for (const [i, chunk] of chunks.entries()) {
  ...
  console.log(`charStart: ${chunk.charStart}, charEnd: ${chunk.charEnd}, tokenCount: ${chunk.tokenCount}`);
